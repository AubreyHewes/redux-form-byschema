...
export default class Renderer {
  ...
  renderChunk = (path, schema, value) => {
    ...
    const id = `root-${path.length ? `${path.join("-")}-` : ""}${propName}`;

    const classNames = ["schema-property", `schema-property-${className}`, `schema-datatype-${schema.type}`];
    ...
    return createElement(Field, {
      ...
      children: schema.enum.map((value, idx) =>
        createElement("option", {
          value: value,
          children: [schema.enum_titles && schema.enum_titles.get(idx) ? schema.enum_titles.get(idx) : value]
        })
      )
    });
  };
  ...
  renderBooleanEnum = (schema, path, value, id, name) =>
    createElement("div", {
      ...
      children: schema.enum.map((itemValue, idx) =>
        createElement("label", {
          className: this.options.radioGroupInlineClass,
          children: [
            createElement(Field, {
              key: name + itemValue,
            ...
            schema.enum_titles && schema.enum_titles.get(idx) ? schema.enum_titles.get(idx) : itemValue
          ]
        })
      )
    });
